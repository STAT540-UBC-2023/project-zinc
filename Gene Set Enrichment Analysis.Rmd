...
```{r}
# Create the heatmap
ggplot(kegg.fgsea, aes(x = pathway, y = 1, fill = NES)) +
  geom_tile() +
...
y <- mutate(enriched_rc_genes, ordering = abs(NES)) %>%
        arrange(desc(ordering)) 

# Visualize the results using gseaplot
gseaplot(enriched_rc_genes, 
         title = "Reactome Pathway Enrichment Analysis",
         top_term = 10, 
         yaxis_scale = "log2")

enrichMap(enriched_rc_genes, layout=igraph::layout.kamada.kawai, vertex.label.cex = 1)
