...
sorted_genes %>% 
  arrange(adj.P.Val) %>% 
  ggplot(aes(1:nrow(sorted_genes), -log10(adj.P.Val)))
    # geom_point(stroke=0) +
...
```{r}
#  extract the numbers from the gene_set_size column in the GO_significant_pathways data frame. 
GO_significant_pathways$gene_set_size_num <- as.numeric(str_extract(GO_significant_pathways$gene_set_size, "\\d+"))
```
