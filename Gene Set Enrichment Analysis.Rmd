...
ensemble_genes <- getBM(attributes=c("ensembl_gene_id", "affy_hugene_1_0_st_v1", "hgnc_symbol", "entrezgene_id", "transcript_length", "chromosome_name"),
...
ensembl_genes <-
    ensemble_genes %>%  
    dplyr::rename(probe = affy_hugene_1_0_st_v1) %>%   # Change the var. name
...
ensembl_genes <-
    ensemble_genes[order(ensemble_genes$transcript_length, decreasing = TRUE),
                   head(.SD, 1),
                   by = .(probe, gene_symbol, chr)]

ensembl_genes <- ensembl_genes %>%
  group_by(probe, gene_symbol, chr) %>%
...
There are 80 significant pathways.
...
```{r}
filtered_genes <- ensembl_geneIDs %>% filter(P.Value < 0.05) 
